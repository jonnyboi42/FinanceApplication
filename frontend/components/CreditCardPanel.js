...
import UncategorizedReviewCredit from "./UncategorizedReviewCredit";
...
export default function CreditCardPanel() {
  ...
  useEffect(() => {
    async function fetchPeriods() {
      const res = await fetch("http://localhost:8000/credit/available-statement-periods");
    ...
    async function fetchData() {
      try {
        const [summaryRes, uncategorizedRes] = await Promise.all([
          fetch(`http://localhost:8000/credit/summary?statement_period=${encodeURIComponent(selectedPeriod)}`),
          fetch(`http://localhost:8000/credit/uncategorized?statement_period=${encodeURIComponent(selectedPeriod)}`)
        ]);
  ...
  const handleDelete = async (txId) => {
    ...
    try {
      await fetch(`http://localhost:8000/credit/delete-transaction/${txId}`, {
        method: "DELETE",
      });
      refreshData();
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const res = await fetch("http://localhost:8000/credit/upload", {
        method: "POST",
        body: formData,
      });
  ...
  const fetchCategoryTransactions = async (category) => {
    const res = await fetch(
      `http://localhost:8000/credit/transactions?statement_period=${encodeURIComponent(selectedPeriod)}&category=${encodeURIComponent(category)}`
    );
    const json = await res.json();
  ...
  return (
    <div className="p-6 text-white bg-black/30 rounded-xl border border-purple-400 shadow-lg">
      <div className="mb-10">
        <label htmlFor="credit-upload" className="block mb-4 text-lg font-semibold text-purple-300">
      ...
      <select
        className="border border-gray-300 p-2 mb-6 rounded text-white focus:outline-none focus:ring-2 focus:ring-blue-400"
      ...
      {selectedCategory && (
        <div className="mt-6 bg-gray-800 p-4 rounded">
          <div className="flex justify-between items-center mb-2">
            <h3 className="text-lg font-bold">Transactions in {selectedCategory}</h3>
            <button
      ...
      {uncategorized.length > 0 && (
        <UncategorizedReviewCredit transactions={uncategorized} onCategoryUpdate={refreshData} />
      )}
    </div>
  );
}
